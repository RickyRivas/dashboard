<script>
	...
	const { panels: rPanels, activePanel: rActivePanel } = api.getReactiveState();
	...
	function promptOk() {
		...
		if (prompt.add) {
			...
			if (initialName !== name) api.exec('rename-file', { id: prompt.item.id, name });
		}
	...
	setContext('filemanager-modals', {
		showPrompt(config) {
			initialName = value =
				config.item.name ||
				(config.item.type === 'folder' ? _('New folder') : `${_('New file')}.txt`);
			prompt = { ...config };
		},
		showConfirm(config) {
			confirm = { ...config };
...
{#if prompt}
	<Modal title={_(`Enter ${prompt.item.type} name`)} onconfirm={promptOk} oncancel={closePrompt}>
